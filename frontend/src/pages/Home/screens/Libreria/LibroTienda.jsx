...
import LibroDetalleScreen from './LibroDetalleScreen';
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-20 text-center">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.4, 0, 0.2, 1] }}>
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full mb-4 bg-white/10 backdrop-blur-sm border border-white/20">
            ...
            <span className="text-xs font-bold tracking-widest uppercase text-white/90">Biblioteca y cultura boliviana</span>
          </div>
          ...
          <p className="text-sm md:text-base mt-4 max-w-2xl mx-auto text-white/80 font-light leading-relaxed">
            Obras que llevan la memoria cultural de Bolivia — elegidas con cuidado, hechas para perdurar.
          </p>

          <div className="flex items-center justify-center gap-2 mt-4 text-xs text-white/70">
            <SparklesIcon className="w-4 h-4 text-amber-400" />
            <span>{totalItems} libros disponibles ahora</span>
          </div>
        ...
        <div className="relative w-full bg-gray-100 dark:bg-gray-800">
          ...
          <div className="absolute top-4 left-4 flex gap-2 z-10">
            <span
              ...
              style={{ background: stockBadge.bg }}
            >
              {stockBadge.text}
            </span>
          </div>
          ...
          <div className="absolute bottom-4 right-4 z-10">
            {mostrarDescuento ? (
              <div className="flex flex-col items-end gap-1">
                <span className="px-4 py-2 rounded-full text-base font-bold bg-gradient-to-r from-red-500 to-red-600 text-white shadow-lg shadow-red-500/30 animate-pulse">
                  Bs. {parseFloat(precioConDescuento).toFixed(2)}
                </span>
                <span className="text-xs line-through text-white/70 bg-black/50 px-2 py-0.5 rounded-full">
                  Bs. {parseFloat(libro.precio).toFixed(2)}
                </span>
                <span className="text-xs font-bold bg-red-600 text-white px-2.5 py-1 rounded-full shadow-lg shadow-red-500/50 animate-pulse">
                  -{descuentoPorcentaje}% OFF
                </span>
              </div>
            ) : (
              <span className="px-4 py-2 rounded-full text-sm font-bold bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-lg">
                Bs. {parseFloat(libro.precio).toFixed(2)}
              </span>
            )}
          </div>
        ...
        <div className="p-5 flex-grow flex flex-col">
          <div className="flex items-start justify-between mb-2">
            <div className="flex-1">
              <h3 className={`text-base font-display font-semibold transition-colors line-clamp-2 ${isDark ? 'text-white group-hover:text-blue-400' : 'text-gray-800 group-hover:text-blue-600'}`}>
          ...
          <div className="mt-3 flex items-center justify-between">
            <div className={`flex items-center gap-2 text-sm ${stock === 0 ? 'text-red-400' : stock < 10 ? 'text-amber-400' : 'text-emerald-400'}`}>
              <TagIcon className="w-4 h-4" />
              <span className="font-semibold">Stock: {stock} uds.</span>
            </div>
...
// ─── Sidebar de filtros optimizada ──────────────────────────────────────
const FiltersSidebar = ({ filters, onFilterChange, categories, authors, repositorios, isOpen, onClose }) => {
      ...
      >
        <label className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 rounded-lg px-2 py-1.5 transition-colors">
          <input 
            type="radio" 
            name="autor" 
            checked={filters.autor === 'todos'} 
            onChange={() => onFilterChange('autor', 'todos')} 
          />
          <span className="text-sm text-gray-600 dark:text-gray-400">Todos</span>
        </label>
        {authors.map((autor) => (
          <label key={autor} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 rounded-lg px-2 py-1.5 transition-colors">
            <input 
              type="radio" 
              name="autor" 
              checked={filters.autor === autor} 
              onChange={() => onFilterChange('autor', autor)} 
            />
            <span className="text-sm text-gray-600 dark:text-gray-400">{autor}</span>
          </label>
        ))}
      </FilterSection>
...
const Breadcrumb = () => (
  <div className="flex items-center gap-1.5 text-xs text-gray-400 dark:text-gray-500 mb-4">
    ...
    <span>Tienda</span>
    <ChevronRightIcon className="w-3 h-3" />
    <span className="font-medium" style={{ color: P.turquesa }}>Libros</span>
  </div>
);
...
const ActiveFilterChips = ({ filters, onFilterChange, repositorios }) => {
  ...
  if (filters.autor !== 'todos') chips.push({ key: 'autor', label: filters.autor, clear: () => onFilterChange('autor', 'todos') });
  ...
  <div className="mt-8 mb-8 rounded-2xl overflow-hidden relative">
    ...
    <div className="relative p-6 md:p-8 text-center">
      ...
      <p className="text-sm text-white/80 mb-4 max-w-md mx-auto">
        Suscríbete para conocer las nuevas publicaciones y promociones exclusivas.
      </p>
...
const LibroTienda = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (librosRes.ok) setLibros(librosData.libros || []);
  ...
  const categories = useMemo(
    () => Array.from(new Set(libros.map((l) => l.categoria_nombre).filter(Boolean))),
    [libros]
  );

  const authors = useMemo(
    () => Array.from(new Set(libros.map((l) => l.autor_nombre_completo).filter(Boolean))),
    [libros]
  );

  const filteredLibros = useMemo(() => {
    let result = libros.filter((l) => {
      if (filters.search && !l.titulo_libro?.toLowerCase().includes(filters.search.toLowerCase()) &&
          !l.autor_nombre_completo?.toLowerCase().includes(filters.search.toLowerCase())) return false;
      if (filters.categoria !== 'todos' && l.categoria_nombre !== filters.categoria) return false;
      if (filters.autor !== 'todos' && l.autor_nombre_completo !== filters.autor) return false;
      if (filters.precioMin && l.precio < parseFloat(filters.precioMin)) return false;
      if (filters.precioMax && l.precio > parseFloat(filters.precioMax)) return false;
      const stock = l.stock_total || 0;
      if (filters.disponibilidad === 'disponible' && stock === 0) return false;
      if (filters.disponibilidad === 'agotado' && stock > 0) return false;
      
      if (filters.repositorio !== 'todos') {
        const tieneStock = l.repositorios_disponibles?.some(r => r.id_repositorio === filters.repositorio && r.stock > 0);
    ...
    switch (sortBy) {
      ...
      case 'precio_desc':
        ...
        break;
      case 'nombre_asc':
        result = [...result].sort((a, b) => (a.titulo_libro || '').localeCompare(b.titulo_libro || ''));
        break;
      default:
        break;
    }
    return result;
  }, [libros, filters, sortBy]);
  ...
  if (selectedLibro) {
    return <LibroDetalleScreen libro={selectedLibro} onBack={() => setSelectedLibro(null)} />;
  }
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex gap-8 pt-8">
              ...
              <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                {[1, 2, 3, 4, 5, 6].map((i) => <LibroCardSkeleton key={i} />)}
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <>
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
        <HeroLibros totalItems={filteredLibros.length} />
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-4 mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      <span className="font-semibold text-gray-700 dark:text-gray-300">{filteredLibros.length}</span> resultados
                    </p>
                  </div>
                ...
                {filteredLibros.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-16 text-center bg-white dark:bg-gray-900 rounded-2xl border" style={{ borderColor: `${P.turquesa}22` }}>
                    ...
                    <h3 className="text-xl font-medium font-display text-gray-700 dark:text-gray-300">No se encontraron libros</h3>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                    <AnimatePresence mode="wait">
                      {filteredLibros.map((libro) => (
                        <LibroCard key={libro.id_producto} libro={libro} onOpen={setSelectedLibro} />
                      ))}
                    </AnimatePresence>
                  </div>
                )}
