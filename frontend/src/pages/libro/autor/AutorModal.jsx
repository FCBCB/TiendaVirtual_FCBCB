...
const AutorModal = ({ isOpen, onClose, onSave, autor, isEditing }) => {
  ...
  useEffect(() => {
    if (autor && isEditing) {
      setFormData({
        ...
        activo: autor.activo !== undefined ? autor.activo : true
      });
    } else {
      setFormData({
        nombre: '',
  ...
  const inputCls = `w-full px-4 py-2.5 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 transition-all
            ...
            <div className={`sticky top-0 flex items-center justify-between p-6 border-b z-20
              ...
              <div className="flex items-center gap-3">
                <div className="p-2 bg-gradient-to-r from-cyan-500/20 to-cyan-600/20 rounded-xl">
                  <UserIcon className={`w-6 h-6 ${isDark ? 'text-cyan-400' : 'text-cyan-600'}`} />
                </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                <label className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                  Autor activo (visible en la aplicación)
                </label>
              </div>
              ...
              <div className="flex gap-3 pt-4 border-t border-gray-200 dark:border-white/10">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="flex-1 px-4 py-2.5 rounded-lg bg-gradient-to-r from-cyan-500 to-cyan-600 text-white font-semibold hover:shadow-lg transition-all duration-300 disabled:opacity-50"
                >
